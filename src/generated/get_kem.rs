        ...
        if check_key_id(&KYBER768, algorithm_id) {
            return Ok(&KYBER768);
        } else 

        if check_key_id(&KYBER1024, algorithm_id) {
            return Ok(&KYBER1024);
        } else 
        ...
        if check_key_id(&SIKEP434COMPRESSED, algorithm_id) {
        ...
        if check_key_id(&SIKEP610COMPRESSED, algorithm_id) {
